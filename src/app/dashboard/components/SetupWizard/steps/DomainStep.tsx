...
export const DomainStep: React.FC<DomainStepProps> = ({ initialValue = '', onProceed }) => {
  ...
  return (
    ...
    >
      ...
      <motion.form variants={itemVariants} onSubmit={handleSubmit} className="max-w-md mx-auto space-y-4">
        <div className="relative group">
          ...
          />
          <div className="absolute inset-[1px] rounded-lg bg-zinc-900/90 backdrop-blur-sm" />
          ...
          <input
            ...
            className="relative w-full px-4 py-3 rounded-lg bg-transparent border-transparent focus:outline-none transition-all duration-200"
          />
          ...
          <Globe className="absolute right-12 top-1/2 transform -translate-y-1/2 text-zinc-500 w-5 h-5 z-10" />

          <div className="absolute right-2 top-1/2 -translate-y-1/2 z-20">
            <div className="absolute inset-0 -left-[1px] bg-zinc-900 rounded-md overflow-hidden">
              <motion.div
            ...
            <button
              ...
              className="relative z-0 flex items-center overflow-hidden whitespace-nowrap rounded-md border-[1px] 
                border-zinc-900 px-4 py-1.5 font-medium text-neutral-300 transition-all duration-300
        ...
        <AnimatePresence>
          {isInputFocused && (
            <motion.div
              ...
              className="bg-zinc-900/50 rounded-lg p-3 border border-zinc-800/50 backdrop-blur-sm"
            >
              <div className="grid grid-cols-3 gap-4 ">
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full transition-colors duration-200 ${getActiveDot() === 'white' ? 'bg-white' : 'bg-white/20'}`} />
                  <span className={`text-sm transition-colors duration-200 ${getActiveDot() === 'white' ? 'text-white' : 'text-white/40'}`}>Not started</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full transition-colors duration-200 ${getActiveDot() === 'invalid' ? 'bg-red-500' : 'bg-red-500/20'}`} />
                  <span className={`text-sm transition-colors duration-200 ${getActiveDot() === 'invalid' ? 'text-red-500' : 'text-red-500/40'}`}>Invalid format</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full transition-colors duration-200 ${getActiveDot() === 'valid' ? 'bg-green-500' : 'bg-green-500/20'}`} />
                  <span className={`text-sm transition-colors duration-200 ${getActiveDot() === 'valid' ? 'text-green-500' : 'text-green-500/40'}`}>Valid domain</span>
                </div>
              </div>
            </motion.div>
          )}
