export interface ChartDataPoint {
  ...
  export const timeRangeConfigs: Record<
    TimeRange,
    { minUsers: number; maxUsers: number; avgLoginsPerUser: number; loginsVariance: number }
  > = {
    '24h': { minUsers: 2000, maxUsers: 5000, avgLoginsPerUser: 3.5, loginsVariance: 0.3 },
    '7d': { minUsers: 500, maxUsers: 1500, avgLoginsPerUser: 5.0, loginsVariance: 0.3 },
    '30d': { minUsers: 400, maxUsers: 800, avgLoginsPerUser: 12.0, loginsVariance: 0.25 },
    '90d': { minUsers: 300, maxUsers: 600, avgLoginsPerUser: 25.0, loginsVariance: 0.25 },
    '6m': { minUsers: 200, maxUsers: 400, avgLoginsPerUser: 40.0, loginsVariance: 0.2 },
    '1y': { minUsers: 100, maxUsers: 300, avgLoginsPerUser: 60.0, loginsVariance: 0.2 },
    lifetime: { minUsers: 30, maxUsers: 100, avgLoginsPerUser: 100.0, loginsVariance: 0.2 },
  };
  
  // Generate a formatted time label for each interval based on the time range
  export const formatTimeLabel = (index: number, timeRange: TimeRange): string => {
    switch (timeRange) {
      case '24h':
  ...
  export const generateDataPoint = (
    ...
    const users = Math.floor(Math.random() * (config.maxUsers - config.minUsers) + config.minUsers);
    ...
    const userChange =
      previousPoint && previousPoint.users > 0
        ? (((users - previousPoint.users) / previousPoint.users) * 100).toFixed(2) + "%"
        : null;
  
    const loginChange =
      previousPoint && previousPoint.logins > 0
        ? (((logins - previousPoint.logins) / previousPoint.logins) * 100).toFixed(2) + "%"
        : null;
  ...
  export const generateTimeBasedData = (timeRange: TimeRange): ChartDataPoint[] => {
  ...
  export const ensureCompleteData = (
    ...
    for (let i = 0; i < intervals; i++) {
      ...
      if (existingPoint) {
        ...
        const previousPoint =
          fullData[fullData.length - 1] || { users: 0, logins: 0, time: timeLabel };
        fullData.push({
          time: timeLabel,
