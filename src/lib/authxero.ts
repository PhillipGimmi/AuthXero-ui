...
export class AuthXeroClient {
  ...
  constructor(config: { baseUrl?: string; logger?: Console } = {}) {
    ...
    this.requestLogger.info('AuthXeroClient initialized', { baseUrl: this.baseUrl });
  }
  ...
  private async handleResponse<T>(response: Response, requestId: string): Promise<T> {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        this.requestLogger.error('Error response received', { ...responseDetails, error: errorData });
        ...
        this.requestLogger.error('Failed to parse error response', { error: e, response: responseDetails });
        throw new AuthXeroError(
    ...
    try {
      ...
      this.requestLogger.debug('Response successfully parsed', { requestId, hasData: !!data });
      return data;
    } catch (e) {
      this.requestLogger.error('Failed to parse success response', { error: e, response: responseDetails });
      throw new AuthXeroError(
  ...
  private async makeRequest<T>(endpoint: string, options: RequestInit): Promise<T> {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        this.requestLogger.warn('Rate limit exceeded', { requestId, retryAfter });
        throw new AuthXeroError(
      ...
      this.requestLogger.info('Request completed', { requestId, durationMs: Math.round(duration) });
  ...
  async signup(credentials: SignUpCredentials): Promise<AuthXeroLoginResponse> {
    ...
    try {
      if (!credentials.name?.trim() || !credentials.email?.trim() || !credentials.password) {
        throw new AuthXeroError('All fields are required', 400, 'VALIDATION_ERROR');
      }
      ...
      if (error instanceof AuthXeroError) {
        if (error.isConflict()) {
          throw new AuthXeroError('Account already exists', 409, 'EMAIL_EXISTS');
        }
        throw error;
      }
  ...
  async verifyEmail(code: string, token: string): Promise<EmailVerificationResponse> {
    ...
    try {
      if (!code?.trim() || !token?.trim()) {
        throw new AuthXeroError('Verification code and token are required', 400, 'VALIDATION_ERROR');
      }
      ...
      if (error instanceof AuthXeroError) {
        ...
        if (error.isRateLimited()) {
          throw new AuthXeroError('Too many attempts, please try again later', 429, 'TOO_MANY_ATTEMPTS');
        }
        throw error;
      }
  ...
  async verifySession(token: string): Promise<AuthXeroUser> {
    ...
    try {
      ...
      throw new AuthXeroError('Session verification failed', 500, 'VERIFICATION_ERROR');
    } finally {
      this.requestLogger.groupEnd();
    }
  }
  ...
  async refreshToken(refreshToken: string): Promise<TokenResponse> {
    ...
    try {
      if (!refreshToken?.trim()) {
        throw new AuthXeroError('Valid refresh token required', 401, 'INVALID_TOKEN');
      }
