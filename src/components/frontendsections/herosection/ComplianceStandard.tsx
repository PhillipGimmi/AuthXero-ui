...
const ComplianceStandard: React.FC<ComplianceStandardProps> = ({ 
  ...
  return (
    <motion.div 
      layout
      className={`relative ${isExpanded ? 'col-span-3' : ''}`}
    >
      <motion.div
        layout
        onClick={onToggle}
      ...
      >
        ...
        {!isExpanded && (
          <div 
            className="absolute inset-0 bg-orange-500 -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-in-out"
          />
        )}
        ...
        <motion.div layout className="p-6 relative z-10">
          <motion.p layout className="text-xl text-white font-light text-center">
            {standard}
          </motion.p>
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                ...
                >
                  <div>
                    <h3 className="text-orange-400 text-sm uppercase tracking-wider mb-2">About</h3>
                    <p className="text-gray-300 text-lg">{info.description}</p>
                  </div>
                  <div>
                    <h3 className="text-orange-400 text-sm uppercase tracking-wider mb-2">How We Help</h3>
