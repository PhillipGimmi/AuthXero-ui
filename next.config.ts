...
async function checkBackendHealth() {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_AUTHXERO_URL}/health`,
    );
    if (response.ok) {
      ...
      console.error(
        'Failed to confirm backend health. Status:',
        response.status,
      );
    }
  } catch (error) {
...
const nextConfig: NextConfig = {
  ...
  env: {
    ...
    NEXT_PUBLIC_SECURE_COOKIES:
      process.env.NEXT_PUBLIC_SECURE_COOKIES ?? 'false',
  },
  async headers() {
    return [
      {
